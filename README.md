...
Two lines of context ride beside the telemetry when they apply. **Push discipline:** at a checkpoint or completion with uncommitted changes or unpushed commits, the hook tells the model to commit (message ending in the ticket id) and `git push -u origin <branch>` — the dashboard links a ticket to its branch, commits and diff only from what is on the remote. **Worktree awareness:** when `git worktree list` shows other worktrees on the repository (the local sign that other agents are working here), the SessionStart context from the main worktree — or from `main`/`master` anywhere — says so and gives the one-liner for a worktree per ticket (`git worktree add ../<repo>-wp-NNNN -b wp-NNNN-<slug>`, `npm ci` inside it, never junction `node_modules`); a claim made on `main` gets "branch or worktree before editing", and a checkpoint/completion with work sitting on `main` gets "another agent may be merging too". Inside a worktree of its own on its own branch the hook says nothing.
